...
def guild_trace_init():
    ...
    f = open("Audi_Member.csv", 'w')
    f.write("캐릭명,박제 링크,가입 상태\n")
...
def csv_to_excel():
    ...
    wb = Workbook()
    ws = wb.active
    with open('Audi_Member.csv', 'r', encoding='utf8') as f:
...
def record_trace():
    ...
    #이제 데이터 비교해서 값 바꿀 예정
    excel_new = pd.read_excel('Audi_Test_'+str(count)+'.xlsx')
    ...
    if df_added.empty:
        ...
        for i in df_added.index:
            print("   %s |    %s" % (koreaFormat(df_added.loc[i, '캐릭명'], 20), koreaFormat(df_added.loc[i, '가입 상태'], 10)))
...
#csv_to_excel()
#guild_trace_init()
schedule.every().hours.do(trace_start)
